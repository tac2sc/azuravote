...
<ul>
  ...
  <li><b>Secure song info fetching/displaying</b> for external mounted streams (other hosts) via local endpoint. Click this .gif:</li>
</ul>
...
- Chat is enabled by default with `CHAT_ENABLED=true`. Set it to `false` to disable both chat API endpoints.
